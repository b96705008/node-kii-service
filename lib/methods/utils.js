...
function hasOperator (value) {
	return typeof value === 'object' && value.operator
}
...
module.exports = function (Kii) {
	...
	Kii.prototype.buildListQueryBody = function (queryParams, fields, opts) {
		...
		if (bucketOpts) {
			...
			if (schema) queryParams = validator.parseQuery(schema, queryParams, fields);
		}
		...
		fields.forEach(function (field) {
			...
			if (value === undefined) {
				...
				subClauses = operators.buildOperatorClauses(field, value);
		...
		console.log(util.inspect(queryBody, false, null));
