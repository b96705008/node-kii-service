...
module.exports = function (Kii) {
	...
	Kii.prototype.fullUpdateObject = function (bucketOpts, objectID, body, cb) {
		var requrl = this.getDataReqURLPrefix(bucketOpts) + 'objects/' + objectID,
			headers = {
				Authorization: 'Bearer ' + this.adminToken.access_token,
				'content-type': 'application/vnd.' + this.APP_ID + '.mydata+json'
			};
		this.requestJSON('PUT', requrl, headers, body, cb);
	};
	...
	Kii.prototype.deleteObject = function (bucketOpts, objectID, cb) {
		var that = this,
			requrl = this.getDataReqURLPrefix(bucketOpts) + 'objects/' + objectID,
			headers = {
				Authorization: 'Bearer ' + this.adminToken.access_token
			},
			schema = this.getSchema(bucketOpts);

		if (!schema || !schema.deletePreHook || !(typeof schema.deletePreHook === 'function')) {
			return this.requestJSON('DELETE', requrl, headers, null, cb);
		}

		async.waterfall([
			function (callback) {
				schema.deletePreHook(objectID, function (err) {
					callback(err || null);
				});
			},
			function (callback) {
				that.requestJSON('DELETE', requrl, headers, null, callback);
			}
		], cb);
	};
